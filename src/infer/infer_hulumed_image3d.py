...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Notebook-style Hulu-Med Qwen3 JSON inference for 3D NIfTI cases"
    )
    ...
    parser.add_argument(
        "--load-4bit",
        action="store_true",
        help="使用 bitsandbytes NF4 量化加载 LLM，降低显存（需安装 bitsandbytes；32B 常用）",
    )
    parser.add_argument("--nii-num-slices", type=int, default=160, help="3D NIfTI 采样切片数")
    parser.add_argument(
        "--nii-slice-axis",
        default="axial",
...
def resolve_visual_path(item, image_root):
    for key in ("videos", "video", "images", "image"):
        if key not in item:
            continue
        value = item[key]
        if isinstance(value, list):
            if not value:
                raise ValueError(f"{key} 为空列表")
            normalized = [normalize_path(v, image_root) for v in value]
            return normalized[0] if len(normalized) == 1 else normalized
        return normalize_path(value, image_root)
    raise KeyError("未找到视觉路径字段，期望 image/images/video/videos 之一")


def has_visual_field(item):
    """判断样本是否包含图像/视频字段，用于区分多模态与纯文本样本。"""
    for key in ("videos", "video", "images", "image"):
...
def parse_question(item):
    ...
    return item.get(
        "text",
        "This is a medical 3D scenario. Please generate a medical report for the given 3D medical images, including both findings and impressions.",
    )


def build_conversation(question, num_frames):
    return [
        {
            "role": "user",
            "content": [
...
def build_text_conversation(question):
    return [
        {
            "role": "user",
            "content": [
...
def _device_map_for_load(device: str, load_4bit: bool):
    """4bit 大模型用 auto 分卡；否则整模型放在 --device 指定单卡，避免 auto 切片与 inputs.to(device) 设备不一致。"""
    if load_4bit:
        return "auto"
...
def load_model_and_processor(model_path, device, attn_implementation, load_4bit=False):
    disable_torch_init()
    model_name = get_model_name_from_path(model_path)
    device_map = _device_map_for_load(device, load_4bit)
    ...
    # 单卡 device_map 已放置权重；4bit+auto 由 accelerate 管理，均勿再 .to(device)
    return tokenizer, model, processor


def resolve_pixel_dtype(model, pixel_values):
    """视觉输入 dtype 与视觉编码器权重一致；4bit 加载时常为 fp16，避免与强制 bf16 的 conv2d 冲突。"""
    ...
    try:
        ...
        if vision_encoder is not None:
            if hasattr(vision_encoder, "dtype") and vision_encoder.dtype is not None:
                return vision_encoder.dtype
            p = next(vision_encoder.parameters(), None)
            if p is not None:
                return p.dtype
    except Exception:
        pass
...
def generate_one_case(model, tokenizer, processor, slices, conversation, device, do_sample, temperature, max_new_tokens):
    modal = "video"
    inputs = processor(
        images=[slices],
        text=conversation,
    ...
    if "pixel_values" in inputs:
        target_dtype = resolve_pixel_dtype(model, inputs["pixel_values"])
...
def generate_text_only(model, tokenizer, processor, conversation, device, do_sample, temperature, max_new_tokens):
    modal = "text"
    inputs = processor(
        text=conversation,
        merge_size=1,
...
def replace_assistant_content(item, output_text):
    """保持原样本结构，仅将 messages 中 assistant 的 content 替换为预测结果。"""
...
def main():
    ...
    for idx, item in enumerate(data, start=1):
        try:
            ...
            print(f"[{idx}/{len(data)}] 跳过，问题解析失败: {exc}")
            continue

        if has_visual_field(item):
            try:
                visual_path = resolve_visual_path(item, args.image_root)
            except Exception as exc:
                print(f"[{idx}/{len(data)}] 跳过，视觉路径解析失败: {exc}")
                continue

            if isinstance(visual_path, list):
                if len(visual_path) != 1:
                    print(f"[{idx}/{len(data)}] 跳过，当前脚本仅支持单个 3D 文件: {visual_path}")
                    continue
                visual_path = visual_path[0]

            if not os.path.exists(visual_path):
                print(f"[{idx}/{len(data)}] 跳过，文件不存在: {visual_path}")
                continue

            print(f"[{idx}/{len(data)}] 正在处理 (多模态): {visual_path}")
            slices = load_images(
                visual_path,
                nii_num_slices=args.nii_num_slices,
                nii_slice_axis=args.nii_slice_axis,
            )
            if not slices:
                print(f"[{idx}/{len(data)}] 跳过，无法从 NIfTI 加载切片: {visual_path}")
                continue

            conversation = build_conversation(question, len(slices))
            ...
            print(f"[{idx}/{len(data)}] 正在处理 (纯文本): {sample_id}")
            conversation = build_text_conversation(question)
            output_text = generate_text_only(
                model=model,
                tokenizer=tokenizer,
                processor=processor,
                conversation=conversation,
